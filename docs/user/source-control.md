...
- Lyn Code can suggest titles and descriptions based on your commits
...
1. Install the GitHub CLI on the machine running Lyn Code:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Lyn Code and verify GitHub shows as authenticated
...
If both are set, the access token wins. Restart Lyn Code and verify the connection in **Source
Control settings**.
...
**Git is required** – Lyn Code uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Lyn Code (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
