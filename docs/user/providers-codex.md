...
This guide is for people who want to use more than one Codex account in Lyn Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Lyn Code](./install.md).
...
In Lyn Code Settings, name it something obvious:
...
In Lyn Code Settings, add another Codex provider:
...
Lyn Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Lyn Code treats it
