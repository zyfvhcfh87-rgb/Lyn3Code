  ...
  switch (command.type) {
    ...
    case "analytics.event.record": {
      ...
      if (mission !== null && (project === null || mission.projectId !== project.id)) {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `Analytics event '${command.eventType}' has a mismatched mission and project.`,
        });
      }
      if (command.payload.taskId !== null) {
        if (mission === null) {
          return yield* new OrchestrationCommandInvariantError({
            commandType: command.type,
            detail: `Analytics event '${command.eventType}' cannot reference a task without a mission.`,
          });
        }
        yield* requireMissionTask({
          readModel,
