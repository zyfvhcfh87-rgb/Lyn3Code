...
const makeEventStore = Effect.gen(function* () {
  ...
  const readEventRowsForAggregate = SqlSchema.findAll({
    Request: ReadForAggregateRequestSchema,
    Result: OrchestrationEventPersistedRowSchema,
    execute: (request) =>
      sql`
        SELECT
  ...
  const readForAggregate: OrchestrationEventStoreShape["readForAggregate"] = (
    aggregateKind,
    aggregateId,
    sequenceExclusive = 0,
    limit = DEFAULT_READ_FROM_SEQUENCE_LIMIT,
    ...
    const readPage = (
      ...
      Stream.fromEffect(
        readEventRowsForAggregate({
          ...
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "OrchestrationEventStore.readForAggregate:query",
              "OrchestrationEventStore.readForAggregate:decodeRows",
            ),
          ),
          Effect.flatMap((rows) =>
            Effect.forEach(rows, (row) =>
              decodeEvent(row).pipe(
                Effect.mapError(
                  toPersistenceDecodeError("OrchestrationEventStore.readForAggregate:rowToEvent"),
                ),
              ),
            ),
          ),
    ...
    return readPage(Math.max(0, Math.floor(sequenceExclusive)), normalizedLimit);
  };
