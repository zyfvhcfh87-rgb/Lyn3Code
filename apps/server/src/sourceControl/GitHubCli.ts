...
const DEFAULT_API_MAX_OUTPUT_BYTES = 10_000_000;
...
export class GitHubCliRequestEncodeError extends Schema.TaggedErrorClass<GitHubCliRequestEncodeError>()(
  "GitHubCliRequestEncodeError",
  {
    command: Schema.Literal("gh"),
    cwd: Schema.String,
  },
  ...
  get detail(): string {
    return "GitHub API request body could not be encoded safely.";
  }

  override get message(): string {
    return `GitHub CLI failed in executeApi: ${this.detail}`;
  }
}
...
const API_HEADER_NAME = /^[a-z0-9-]+$/i;
const API_HOSTNAME = /^(?=.{1,259}$)[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?(?::\d{1,5})?$/i;

function apiHeadersAreSafe(headers: Readonly<Record<string, string>>): boolean {
  const entries = Object.entries(headers);
  return (
    entries.length <= 32 &&
    entries.every(
      ([name, value]) =>
        API_HEADER_NAME.test(name) &&
        !FORBIDDEN_API_HEADERS.has(name.toLowerCase()) &&
        value.length <= 4_096 &&
        !value.includes("\r") &&
        !value.includes("\n") &&
        !value.includes("\0"),
...
export const make = Effect.gen(function* () {
  ...
  const executeApi: GitHubCli["Service"]["executeApi"] = (input) => {
    const headers = input.headers ?? {};
    if (
      input.endpoint.length === 0 ||
      input.endpoint.length > 4_096 ||
      input.endpoint.startsWith("-") ||
      input.endpoint.includes("\0") ||
      (input.hostname !== undefined && !API_HOSTNAME.test(input.hostname)) ||
      !apiHeadersAreSafe(headers)
    ) {
      return Effect.fail(new GitHubCliRequestEncodeError({ command: "gh", cwd: input.cwd }));
    }

    let stdin: string | undefined;
    if (input.body !== undefined) {
      try {
        stdin = encodeUnknownJson(input.body);
      } catch {
        return Effect.fail(new GitHubCliRequestEncodeError({ command: "gh", cwd: input.cwd }));
      }
    }
    ...
    return process
      .run({
        ...
        timeoutMs: input.timeoutMs ?? DEFAULT_TIMEOUT_MS,
        maxOutputBytes: input.maxOutputBytes ?? DEFAULT_API_MAX_OUTPUT_BYTES,
